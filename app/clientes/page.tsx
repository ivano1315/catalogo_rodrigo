          ...
          <div className="ml-auto flex gap-2">
            <div className="relative">
              <input
                ...
                className="border border-gray-200 rounded-lg pl-4 pr-8 py-2 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
              {busca && (
                <button onClick={() => setBusca('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-xs">✕</button>
              )}
            </div>
            <button
              onClick={abrirNovo}
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        {carregando ? (
          ...
          <div className="space-y-2">
            {clientes.map(c => (
              <div key={c._id} className="bg-white rounded-xl border border-gray-100 px-5 py-4 flex items-center gap-4 hover:shadow-sm transition-shadow">
                <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-blue-500 font-bold text-sm shrink-0">
                  {c.razaoSocial.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-800 truncate">{c.razaoSocial}</p>
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg text-gray-400 hover:bg-blue-50 hover:text-blue-600 transition-colors flex items-center justify-center text-sm"
                    title="Editar"
                  >✏️</button>
                  <button
                    onClick={() => setDeletandoId(c._id!)}
                    className="w-8 h-8 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 transition-colors flex items-center justify-center text-sm"
                    title="Excluir"
                  >🗑️</button>
                </div>
              </div>
